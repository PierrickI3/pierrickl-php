...
)
{
  ...
  case $ensure
  {
    installed:
    {
      ...
      # Download PHP
      pget {'Download PHP':
        source => 'http://windows.php.net/downloads/releases/php-5.6.14-nts-Win32-VC11-x64.zip',
        target => $cache_dir,
      }

      # Unzip to C:\PHP
      php::unzip {"${cache_dir}/php-5.6.14-nts-Win32-VC11-x64.zip":
        destination => 'C:/PHP',
      ...
      # Add C:\PHP to PATH
      php::path {'C:\PHP':
        require => Php::Unzip["${cache_dir}/php-5.6.14-nts-Win32-VC11-x64.zip"],
      }
